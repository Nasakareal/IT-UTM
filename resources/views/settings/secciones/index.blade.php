...
@section('title', 'TI-UTM - Secciones')
...
<!-- Fila principal con la tarjeta de Secciones -->
<div class="row">
    <div class="col-md-12">
        <!-- Tarjeta principal -->
        <div class="card" style="border-radius: 8px; overflow: hidden;">
            ...
            <div class="card-header d-flex justify-content-between align-items-center" style="background-color: #1976d2;">
                <h3 class="card-title text-white mb-0">Secciones Registrados</h3>
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table id="Secciones" class="table table-bordered table-hover table-sm mb-0">
                        ...
                        <tbody style="background-color: #fff;">
                            @foreach ($secciones as $index => $seccion)
                                <tr>
                                    ...
                                    <td style="text-align: center">{{ $seccion->created_at->format('d-m-Y') }}</td>
                                    <td style="text-align: center">
                                        <div class="btn-group" role="group">
                                            <!-- Vista pública (show) fuera del settings -->
                                            <a href="{{ url('/secciones/' . $seccion->id) }}" class="btn btn-info btn-sm">
                                                <i class="fa-regular fa-eye"></i>
                                            </a>
                                            <!-- Edición en el área de configuración -->
                                            <a href="{{ url('/settings/secciones/' . $seccion->id . '/edit') }}" class="btn btn-success btn-sm">
                                                <i class="fa-regular fa-pen-to-square"></i>
                                            </a>
                                            <!-- Formulario para eliminar -->
                                            <form action="{{ url('/settings/secciones/' . $seccion->id) }}" method="POST" style="display:inline-block;">
                                                @csrf
                                                @method('DELETE')
