...
@section('content')
    ...
    <div class="row gx-4 gy-4">
      @foreach(collect($documentos)->groupBy(function($d){ return $d['materia'].'|'.$d['grupo']; }) as $key => $docs)
        @php
          list($materia, $grupo) = explode('|', $key);
          $slug     = \Illuminate\Support\Str::slug($materia.'-'.$grupo);
    ...
    @foreach(collect($documentos)->groupBy(function($d){ return $d['materia'].'|'.$d['grupo']; }) as $key => $docs)
      @php
        list($materia, $grupo) = explode('|', $key);
        $slug     = \Illuminate\Support\Str::slug($materia.'-'.$grupo);
        // extraemos las unidades disponibles (por defecto unidad 1 y la unidad actual)
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered">
          <div class="modal-content">
            ...
            <div class="modal-header d-flex align-items-center">
              ...
              <!-- Select de unidades -->
              <select id="unidad_select_{{ $slug }}" class="form-select form-select-sm ms-3" style="width:auto;">
      ...
      <script>
        // Para cada modal, al cambiar la unidad
        document.addEventListener('DOMContentLoaded', function(){
          var sel = document.getElementById('unidad_select_{{ $slug }}');
          sel.addEventListener('change', function(){
            var val = this.value;
            var modal = this.closest('.modal-content');
            // Actualiza título
            modal.querySelector('.unidad-display').textContent = val;
            // Muestra sólo el bloque de la unidad seleccionada
            modal.querySelectorAll('.docs-unidad').forEach(function(div){
    ...
    <script>
      // Script general para cargar el .p12 en base64 (igual que antes)
