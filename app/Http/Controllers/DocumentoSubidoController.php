...
class DocumentoSubidoController extends Controller
{
    public function store(Request $request)
    {
        ...
        $quarter = trim((string)$request->input('quarter_name', ''));
        if ($quarter === '') {
            $c = DB::table('cuatrimestres')
                ->whereDate('fecha_inicio','<=', now())
                ->whereDate('fecha_fin','>=', now())
                ->orderBy('fecha_inicio','desc')
                ->first();
            $quarter = $c->nombre ?? 'SIN CUATRIMESTRE';
        }
        ...
        $certName = $info['subject']['CN']           ?? (Auth::user()->name ?? 'N/A');
        $certRFC  = $info['subject']['serialNumber'] ?? 'N/A';
        ...
        try {
            if ($ext === 'docx') {
                    ...
                    foreach ($section->getElements() as $element) {
                        ...
                        foreach ($element->getRows() as $row) {
                            foreach ($row->getCells() as $cell) {
                                $txt = '';
                                foreach ($cell->getElements() as $e) { if (method_exists($e, 'getText')) $txt .= $e->getText(); }
                                if (stripos($txt, 'Nombre y Firma del Profesor') !== false) {
                                    $cell->addTextBreak();
                                    $cell->addText("Firmado por: {$certName}", ['name'=>'Calibri','size'=>11], ['spaceAfter'=>0]);
                                    $cell->addText("Hash: {$hashOriginal}",    ['name'=>'Calibri','size'=>9],  ['spaceAfter'=>0]);
                                    $done = true; break 3;
                                }
                            }
                        }
                ...
                if ($done) {
                    ...
                    $zipOld = new ZipArchive; $zipNew = new ZipArchive;
            ...
            if (in_array($ext, ['xls','xlsx'])) {
                ...
                Settings::setCacheStorageMethod(CachedObjectStorageFactory::cache_to_phpTemp, ['memoryCacheSize'=>'32MB']);
                $reader      = $ext === 'xlsx' ? new XlsxReader() : new XlsReader();
                $spreadsheet = $reader->load($absPath);
                $sheet       = $spreadsheet->getActiveSheet();
                $done        = false; $label = 'nombre y firma del profesor';

                foreach ($sheet->getRowIterator() as $row) {
                    foreach ($row->getCellIterator() as $cell) {
                        $value = strtolower(trim((string)$cell->getValue()));
                        if (strpos($value, $label) !== false) {
                            $col=$cell->getColumn(); $rowDest=$cell->getRow()+1; $coord=$col.$rowDest;
                            $sheet->setCellValueExplicit($coord, "Firmado por: {$certName}\nHash: {$hashOriginal}", DataType::TYPE_STRING);
                ...
                if (!$done) {
                    foreach ($sheet->getMergeCells() as $range) {
                        ...
                        if (strpos($value, $label) !== false) {
                            ...
                            $col=$m[1]; $rowDest=$m[2]+1; $coord=$col.$rowDest;
                            $sheet->setCellValueExplicit($coord, "Firmado por: {$certName}\nHash: {$hashOriginal}", DataType::TYPE_STRING);
                            $sheet->getStyle($coord)->getAlignment()->setWrapText(true);
                            $done = true; break;
                        }
                ...
                if ($done) {
                    $writer = SpreadsheetIOFactory::createWriter($spreadsheet, $ext === 'xlsx' ? 'Xlsx' : 'Xls');
